...
execute if score $radius wanecroversal.values > $maxRadius wanecroversal.values run execute store result score $radius wanecroversal.values run scoreboard players get $maxRadius wanecroversal.values
...
execute in minecraft:overworld if entity @p[distance=0..] \
    run execute store result storage wanecroversal:border_args diameter int 1 run scoreboard players get $diameter wanecroversal.values
execute in minecraft:overworld unless entity @p[distance=0..] \
    run execute store result storage wanecroversal:border_args diameter int 1 run scoreboard players get $maxRadius wanecroversal.values
